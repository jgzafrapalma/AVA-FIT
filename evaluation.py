...
def get_args():
    parser = argparse.ArgumentParser(description="Get metrics from predictions")
    parser.add_argument("--dataset", type=str, required=True, choices=["avafit", "fit3d"], help="Dataset to evaluate")
    ...
    parser.add_argument("--repetitions", nargs='+', required=False, help="Repetitions to process (only for avafit)")
...
def compute_metrics(v3d_ctx, v3d_hat_ctx, verbose=False):
    """
    Calcula todas las métricas comunes: SMPLX PVE, regiones, SMPL PVE, MPJPE.
    ...
    if v3d_hat_ctx.shape[1] == 10475:
        ...
        if verbose:
            print(f"SMPLX: PVE: {pve_full.mean():.2f} - PA-PVE: {pa_pve_full.mean():.2f}")
        ...
        for region, vertices in segmentation_dict.items():
            ...
            # 1. PVE (Error sin alineación)
            region_pve = ((np.sqrt(((region_v3d - region_v3d_hat) ** 2).sum(-1))) * 1000).mean(axis=1)
            
            # 2. PA-PVE (Usando alineación global)
            region_pa_pve = ((np.sqrt(((region_v3d - region_pa_v3d_hat_global) ** 2).sum(-1))) * 1000).mean(axis=1)

            # Guardamos los errores
            region_errors[region] = {"PVE_SMPLX": region_pve, "PA-PVE_SMPLX": region_pa_pve}

            if verbose:
                print(f"Error para la región {region}: PVE: {region_pve.mean():.2f} - PA-PVE: {region_pa_pve.mean():.2f}")


        # Hands and Feets PVE and PA-PVE
        ...
        if verbose:
            print(f"Hands: PVE: {hands_pve.mean():.2f} - PA-PVE: {hands_pa_pve.mean():.2f}")
        ...
        if verbose:
            print(f"Feets: PVE: {feets_pve.mean():.2f} - PA-PVE: {feets_pa_pve.mean():.2f}")
    ...
    if verbose:
        print(f"SMPL: PVE: {pve.mean():.2f} - PA-PVE: {pa_pve.mean():.2f}")
    ...
    # Build results dict
    metrics = {"MPJPE": mpjpe, "PA-MPJPE": pa_mpjpe, "PVE": pve, "PA-PVE": pa_pve}

    if smplx_pred:
        metrics.update({
            "PVE_SMPLX": pve_full, "PA-PVE_SMPLX": pa_pve_full,
            "PVE_HANDS": hands_pve, "PA-PVE_HANDS": hands_pa_pve,
            "PVE_FEETS": feets_pve, "PA-PVE_FEETS": feets_pa_pve
        })
        metrics["region_errors"] = region_errors

    return metrics


def evaluate_avafit(predictions_path, gt_smplx_path, output_path,
                    participants=None, exercises=None, viewpoints=None,
                    repetitions=None, verbose=False, overwrite=False):
        ...
        for exercise in sorted(EXERCISES):
            ...
            for viewpoint in sorted(VIEWPOINTS):
                ...
                for repetition in sorted(REPETITIONS):
                    ...
                    # Compute metrics
                    metrics = compute_metrics(v3d_ctx, v3d_hat_ctx, verbose=verbose)
                    metrics["paths"] = np.array(paths_selected)
...
def evaluate_fit3d(predictions_path, gt_smplx_path, output_path,
                   participants=None, exercises=None, viewpoints=None,
    ...
    for participant in PARTICIPANT:
        ...
        for exercise in EXERCISES:
            ...
            for viewpoint in VIEWPOINTS:
                ...
                # Store results
                region_errors = metrics.pop("region_errors", None)
                results[participant][exercise][viewpoint] = metrics
                if region_errors is not None:
